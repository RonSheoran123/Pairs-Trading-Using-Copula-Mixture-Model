...
# Function to calculate copula density
# copula_density <- function(u, fitted_copula) {
#   return(dCopula(u, fitted_copula@copula))
# }
...
plot_scatter_with_density <- function(randoms, copula_name) {
  ...
  ggplot(randoms_df, aes(x = V, y = MA)) +
    ...
    labs(title = paste("Scatter Plot of", copula_name),
         x = "Random Samples of V",
         y = "Random Samples of MA") +
    theme_minimal()
}

plot_wireframe <- function(copula_name){
  # Plotting with wireframe2
  wireframe2(copula_name, FUN=dCopula, col.4=adjustcolor("black", alpha.f=0.25), xlab="u1", ylab="u2", zlab="dCopula", main="Clayton Copula")
}
...
# Calculate densities for each fitted copula
# gaussian_density <- copula_density(gaussian_res$density, gaussian_res$copula)
# gumbel_density <- copula_density(gumbel_res$density, gumbel_res$copula)
# clayton_density <- copula_density(clayton_res$density, clayton_res$copula)

# Plot densities for each copula
gaussian_plot <- plot_scatter_with_density(rCopula(1000, gaussian_res$cop@copula),"Gaussian")
gumbel_plot <- plot_scatter_with_density(rCopula(1000, gumbel_res$cop@copula),"Gumbel")
clayton_plot <- plot_scatter_with_density(rCopula(1000, clayton_res$cop@copula),"Clayton")

# # Plot densities for each copula
# gaussian_plot <- plot_wireframe(gaussian_res$cop@copula)
# gumbel_plot <- plot_wireframe(gumbel_res$cop@copula)
# clayton_plot <- plot_wireframe(clayton_res$cop@copula)

# Combine plots into one figure
combined_plot <- grid.arrange(gaussian_plot, gumbel_plot, clayton_plot, nrow=1, top="Wireframe plots for Gaussian, Gumbel and Clayton Copula")
